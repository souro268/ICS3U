...
def Degrees0(plotIT):
    """Draws the image in its original orientation (0-degree rotation)."""
    t.tracer(0, 0)
    b = cols / 2 * thickness
...
def Degrees90(plotIT):
    """Rotates the image 90 degrees clockwise and draws it."""
    t.tracer(0, 0)
    b = cols / 2 * thickness
    for x in range(len(arr)):
        a = -rows / 2 * thickness
...
def Degrees180(plotIT):
    """Rotates the image 180 degrees clockwise and draws it."""
    t.tracer(0, 0)
    b = cols / 2 * thickness
    for x in range(len(arr)):
        a = -rows / 2 * thickness
...
def Degrees270(plotIT):
    """Rotates the image 270 degrees clockwise and draws it."""
    t.tracer(0, 0)
    b = cols / 2 * thickness
    for x in range(len(arr)):
        a = -rows / 2 * thickness
...
colorData = fh.readline().strip()  # Read and clean the first line of the file

rows, cols, numColors, Deletethis = colorData.split()  # Split metadata into variables
